...
try {
    ...
    if ($scripts.Count -ne 1 -or $clients.Count -ne 1 -or $modules.Count -ne 6) {
    ...
    Write-Output 'PASS Rojo build: one server, one client, six modules; no sync test.'
    ...
    if ((Get-Content -Raw src/server/Main.server.luau) -match '\.OnServerEvent\s*:') {
        throw 'Feedback must remain outbound-only.'
    }
    Write-Output 'PASS static boundaries: server-only preview, two Studio guards, outbound-only feedback.'
    if ($LuauDirectory) {
